...
must_haves:
  ...
  key_links:
    ...
    - from: "src/mobile_crawler/domain/droidrun_agent_service.py"
      ...
      via: "capture_pre_state before EXECUTE, verify after wait in RECORD"
      pattern: "capture_pre_state|ActionVerifier|verify\(pre_state"
...
<objective>
Wire the step phase state machine, UI wait predicates, and action verifier into the DroidRun event stream. Each ToolExecutionEvent drives the phase machine through CAPTURE->DECIDE->EXECUTE->RECORD->CHECKPOINT. ActionVerifier captures pre-state before EXECUTE and verifies post-state during RECORD. Adaptive wait timing replaces DroidRun's built-in fixed delays.

Purpose: Per D-01, wrap DroidRun at the action level. This plan is the integration point that connects all components from Plans 01, 02, and 03 into the live crawl loop. SYNC-02 requires post-action verification (ActionVerifier wired into event handler), SYNC-03 requires adaptive wait timing.

Output: ActionVerifier class, DroidRunAgentService event stream consumer with full verify+wait wiring, CrawlerLoop phase event emission, and tests.
</objective>
...
@.planning/phases/03-step-state-machine-ui-sync/03-02-SUMMARY.md
...
<task type="auto">
  <name>Task 2: Wire step phase machine and UIWaitPredicate into DroidRun event stream</name>
  ...
  <action>
    ...
    **Modifications to `src/mobile_crawler/core/crawler_loop.py`:**
    ...
    This uses the existing `_emit_event` pattern (already used for on_crawl_started, on_debug_log, etc.) to forward step phase transitions to all registered listeners. The `_emit_event` callback dispatches to `on_step_phase_transition` on each listener (added by Plan 02).
  </action>
  <verify>
    <automated>cd E:/VS-projects/mobile-crawler/src && python -m pytest tests/ -v --timeout=60</automated>
  </verify>
  <done>
    ...
    - ActionVerifier.capture_pre_state() called BEFORE UI wait (before EXECUTE settles)
    - ActionVerifier.verify(pre_state, tool_name) called AFTER UI wait, in RECORD phase
    - VerificationResult logged at INFO level with step number and action type
...
| Event consumer -> ActionVerifier | Verification wraps StateProvider calls; failures logged but do not halt crawl |
...
| T-03-11 | Denial of Service | ActionVerifier capture_pre_state or verify blocks event loop | mitigate | Both methods are async and wrap all exceptions in try/except with logger.warning; failures do not halt phase transitions |
</threat_model>
...
7. `grep "capture_pre_state" src/mobile_crawler/domain/droidrun_agent_service.py` returns match
8. `grep "verify\(pre_state" src/mobile_crawler/domain/droidrun_agent_service.py` returns match
9. All existing tests still pass: `cd E:/VS-projects/mobile-crawler/src && python -m pytest tests/ -v --timeout=60`
</verification>

<success_criteria>
- ToolExecutionEvent stream drives step phase machine through CAPTURE->DECIDE->EXECUTE->RECORD->CHECKPOINT per action
- ActionVerifier.capture_pre_state() called before UI wait in each step
- ActionVerifier.verify(pre_state, tool_name) called after UI wait in RECORD phase
- VerificationResult logged for every step (INFO level for success, WARNING for failures)
...
- Phase transitions emitted to CrawlerLoop listeners via on_step_phase_transition (method added by Plan 02)
