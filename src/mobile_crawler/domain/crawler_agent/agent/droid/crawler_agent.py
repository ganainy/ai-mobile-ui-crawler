...
from mobile_crawler.domain.crawler_agent.agent.utils.llm_loader import (
    load_agent_llms,
...
from mobile_crawler.domain.crawler_agent.agent.utils.tracing_setup import (
...
from mobile_crawler.domain.crawler_agent.credential_manager import CredentialManager, FileCredentialManager
from mobile_crawler.domain.crawler_agent.log_handlers import CLILogHandler, configure_logging
from mobile_crawler.domain.crawler_agent.mcp.adapter import mcp_to_droidrun_tools
from mobile_crawler.domain.crawler_agent.mcp.client import MCPClientManager
...
class CrawlerAgent(Workflow):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.config = CrawlerConfig(
            ...
            a11y_checks=config.a11y_checks if config else {},
    ...
    async def start_handler(self, ctx: Context, ev: StartEvent) -> FastAgentExecuteEvent | ManagerInputEvent:
        ...
        if self._using_external_agent:
            ...
            if not agent_module:
                from mobile_crawler.domain.crawler_agent.agent.external import list_agents
        ...
        if self._injected_state_provider is not None:
            ...
            self.state_provider = AndroidStateProvider(
                ...
                a11y_checks=self.config.a11y_checks,
