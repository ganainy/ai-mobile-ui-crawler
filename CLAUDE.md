...
- For code structure and file relationships read `docs/code/` (one note per module with classes, functions, imports and imported-by links). It is generated by `scripts/gen_code_notes.py` (no hook: run `.venv312/Scripts/python.exe scripts/gen_code_notes.py` before committing changes to `src/**/*.py`); never edit it by hand except the summary line between the `summary` markers in `_index.md` files.
