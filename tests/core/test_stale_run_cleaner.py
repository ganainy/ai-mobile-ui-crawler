...
@pytest.fixture
def stale_run_cleaner(mock_db_manager, mock_traffic_manager):
...
class TestStaleRunCleanerInit:
    ...
    def test_init_with_all_dependencies(self, mock_db_manager, mock_traffic_manager):
...
class TestCleanupSingleRun:
    ...
    def test_cleanup_single_run(self, stale_run_cleaner, sample_run):
        ...
        stale_run_cleaner._mark_run_as_interrupted = Mock()
        ...
        stale_run_cleaner._mark_run_as_interrupted.assert_called_once_with(sample_run)


class TestStopOngoingRecordings:
    """Tests for _stop_ongoing_recordings method."""

    def test_stop_traffic_capture(self, stale_run_cleaner, sample_run):
        """Test stopping traffic capture when manager is available."""
    ...
    def test_stop_recordings_no_manager(self, stale_run_cleaner, sample_run):
        """Test when traffic capture manager is None."""
        stale_run_cleaner.traffic_capture_manager = None

        # Should not raise exception
        stale_run_cleaner._stop_ongoing_recordings(sample_run)

    def test_stop_recordings_with_failure(self, stale_run_cleaner, sample_run):
        """Test that traffic capture stop failure is handled gracefully."""
...
class TestMarkRunAsInterrupted:
    """Tests for _mark_run_as_interrupted method."""

    def test_mark_run_as_interrupted(self, stale_run_cleaner, sample_run):
        """Test marking a run as INTERRUPTED."""
        conn = stale_run_cleaner.db_manager.get_connection.return_value
        cursor = conn.cursor.return_value

        stale_run_cleaner._mark_run_as_interrupted(sample_run)

        cursor.execute.assert_called_once()
        execute_args = cursor.execute.call_args[0]
        assert "UPDATE runs" in execute_args[0]
        assert "SET status = 'INTERRUPTED'" in execute_args[0]
        assert execute_args[1][0] is not None  # timestamp
        assert execute_args[1][1] == sample_run.id
        conn.commit.assert_called_once()

    def test_mark_run_as_interrupted_with_timestamp(self, stale_run_cleaner, sample_run):
        """Test that timestamp is set when marking as interrupted."""
        conn = stale_run_cleaner.db_manager.get_connection.return_value
        cursor = conn.cursor.return_value
...
class TestIntegration:
    ...
    def test_full_cleanup_flow(self, stale_run_cleaner, sample_run):
        """Test the full cleanup flow from finding to marking as interrupted."""
        conn = stale_run_cleaner.db_manager.get_connection.return_value
        cursor = conn.cursor.return_value
        cursor.fetchall.return_value = [
            (1, "emulator-5554", "com.example.app", None, "RUNNING",
