...
SERVER_ID = "bfovcwfh"
# SMTP Settings for sending test emails
SMTP_SERVER = "smtp.gmail.com"
SMTP_PORT = 465
SENDER_EMAIL = os.environ.get("TEST_EMAIL_SENDER") or "afoda50@gmail.com"
# TEST_EMAIL_SMTP_PASSWORD will be read from .env

class SimpleEmailSender:
    """Standalone sender to avoids broken package imports."""
    def __init__(self):
        self.app_password = os.environ.get("TEST_EMAIL_SMTP_PASSWORD") or os.environ.get("GMAIL_TEST_APP_PASSWORD")
        if not self.app_password:
            # Try reading .env manually
            if os.path.exists(".env"):
                with open(".env", "r") as f:
                    for line in f:
                        if line.startswith("TEST_EMAIL_SMTP_PASSWORD=") or line.startswith("GMAIL_TEST_APP_PASSWORD="):
                            self.app_password = line.split("=", 1)[1].strip().strip('"').strip("'")
                            break
                            
    def send(self, to, subject, body):
        if not self.app_password:
            raise ValueError("TEST_EMAIL_SMTP_PASSWORD not found")
        msg = EmailMessage()
...
def test_live_otp_flow(mailosaur_service, email_sender):
    """Test OTP: Send via SMTP, Receive via Mailosaur."""
...
def test_live_magic_link_flow(mailosaur_service, email_sender):
    """Test Magic Link: Send via SMTP, Receive via Mailosaur."""
