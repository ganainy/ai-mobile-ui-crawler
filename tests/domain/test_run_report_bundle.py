...
from mobile_crawler.domain.report_generator import ReportGenerator, RunFolderMissingError
...
def seeded_run(tmp_path):
    ...
    for sub in ("screenshots", "reports"):
...
def test_generate_writes_analysis_bundle_next_to_html(seeded_run):
    ...
    assert html_path == str(session / "reports" / "run_report.html")
    analysis = session / "reports" / "analysis"
    assert (analysis / "analysis.md").is_file()
    assert (analysis / "steps.jsonl").is_file()
    assert (analysis / "run.json").is_file()


def test_generate_writes_no_separate_json_export(seeded_run):
    db, run_id, session = seeded_run

    ReportGenerator(db).generate(run_id)

    assert sorted(p.name for p in (session / "reports").iterdir()) == ["analysis", "run_report.html"]
    assert not (session / "analysis").exists()


def test_html_links_screenshots_relative_to_its_own_folder(seeded_run):
    db, run_id, session = seeded_run

    html = open(ReportGenerator(db).generate(run_id), encoding="utf-8").read()

    assert 'src="../screenshots/step_1.png"' in html.replace("\\", "/")


def test_generate_passes_the_runs_pcap_and_mobsf_report_to_the_correlator(seeded_run):
    db, run_id, session = seeded_run
    (session / "pcap").mkdir()
    pcap = session / "pcap" / "com.example.health_run1_20260919_100000.pcap"
    pcap.write_bytes(b"")
    (session / "reports" / "mobsf").mkdir()
    mobsf = session / "reports" / "mobsf" / "abc123_report.json"
    mobsf.write_text("{}", encoding="utf-8")

    generator = ReportGenerator(db)
    real_correlate = generator.correlator.correlate
    seen = {}

    def spy(**kwargs):
        seen.update(kwargs)
        return real_correlate(**{**kwargs, "pcap_path": None, "mobsf_path": None})

    generator.correlator.correlate = spy
    generator.generate(run_id)

    assert seen["pcap_path"] == str(pcap)
    assert seen["mobsf_path"] == str(mobsf)


def test_custom_output_moves_only_the_html(seeded_run, tmp_path, monkeypatch):
    db, run_id, session = seeded_run
    monkeypatch.chdir(tmp_path)

    html_path = ReportGenerator(db).generate(run_id, "custom.html")

    assert (tmp_path / "custom.html").is_file() and html_path == "custom.html"
    assert (session / "reports" / "analysis" / "analysis.md").is_file()
    assert not (tmp_path / "analysis").exists()


def test_generate_without_run_folder_fails_instead_of_writing_to_cwd(seeded_run, tmp_path, monkeypatch):
    db, run_id, session = seeded_run
    RunRepository(db).update_session_path(run_id, str(tmp_path / "gone"))
    monkeypatch.chdir(tmp_path)

    with pytest.raises(RunFolderMissingError):
        ReportGenerator(db).generate(run_id)
...
def test_steps_jsonl_has_one_line_per_step_with_relative_screenshot(seeded_run):
    ...
    lines = (session / "reports" / "analysis" / "steps.jsonl").read_text(encoding="utf-8").splitlines()
...
def test_analysis_md_summarises_run_and_stop_reason(seeded_run):
    ...
    text = (session / "reports" / "analysis" / "analysis.md").read_text(encoding="utf-8")
...
def test_run_json_keeps_full_prompts(seeded_run):
    ...
    data = json.loads((session / "reports" / "analysis" / "run.json").read_text(encoding="utf-8"))
...
def problem_run(tmp_path):
    ...
    for sub in ("screenshots", "reports"):
        (session / sub).mkdir(parents=True)
    (session / "reports" / "config_snapshot.json").write_text(
...
def _analysis_md(problem_run) -> str:
    ...
    ReportGenerator(db).generate(run_id)
    return (session / "reports" / "analysis" / "analysis.md").read_text(encoding="utf-8")
...
def _run_json(session):
    return json.loads((session / "reports" / "analysis" / "run.json").read_text(encoding="utf-8"))
...
def test_auto_generate_marks_telemetry_pending_without_calling_the_server(traced_run):
    ...
    assert "pending" in (session / "reports" / "analysis" / "analysis.md").read_text(encoding="utf-8")
...
def test_manual_generate_fetches_telemetry_into_the_report(traced_run):
    ...
    md = (session / "reports" / "analysis" / "analysis.md").read_text(encoding="utf-8")
...
def test_analysis_md_omits_timing_section_without_phase_rows(seeded_run):
    ...
    assert "Timing breakdown" not in (session / "reports" / "analysis" / "analysis.md").read_text(encoding="utf-8")
