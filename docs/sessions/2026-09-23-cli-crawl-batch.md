...
- The main checkout has uncommitted edits to `crawl.py` (pre-run warnings, `RunStatsRepository` wiring) from another session; merging this branch will conflict in `crawl.py` and the batch should call `collect_pre_run_warnings` once at batch start when those land.
