...
- No more working-directory fallbacks: `ReportGenerator.generate` raises `RunFolderMissingError` without a run folder (unless `--output` is given; the bundle then goes next to that HTML; with a run folder `--output` moves only the HTML, as before); `perform_complete_scan` fails with "No run folder ..." instead of writing to `output_data/` in the cwd; `extract_apk_from_device` / `save_*_report` now require their output path. `scan_results_dir` removed. Follow-up at the user's request: `TrafficCaptureManager.start_capture_async` now fails with "No run folder to save the traffic capture in" (before touching PCAPdroid) instead of writing to `output_data/traffic_captures` in the cwd, and `SessionFolderManager.get_subfolder` (which invented `output_data/run_<id>/` for runs without a folder) is deleted; the loop takes the screenshots dir from `RunFolderLayout`.
...
- Migration run for real on `%APPDATA%\mobile-crawler\output_data` at the user's request: 179 run folders, 221 moves, 11 deletes, 0 skips; a second dry run reports nothing left; no `analysis/`, `data/` or `logs/` folders remain.
