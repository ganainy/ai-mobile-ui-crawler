...
App cards provide app-specific guidance to `crawler_agent` agents. They help agents understand how to operate specific apps more effectively.
...
3. **Automatic Loading**: `crawler_agent` automatically loads the appropriate app card based on the current package name
...
3. **Absolute path**:
   ```json
   {"com.google.gm": "/usr/share/crawler_agent/cards/gmail.md"}
   ```
...
2. **From device**:
   - Open the app
   - Run `crawler_agent` with debug mode to see the current package name in logs
...
```python
from mobile_crawler.domain.crawler_agent.config_manager import AppCardLoader, config
